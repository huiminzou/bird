...
track_direction='forward'#forward or backward
days=7
filepath='forward_201708_differ_starttime/' 
...
if track_direction=='backward':
    for i in range(len(data['lon'])):
        ...
        if ind2<0:#the dataset is from (2017,8,1,0,0,0)
            ind3=-ind2#get the index in July        
            ind2=0
        ...
        url_time_201707='http://www.smast.umassd.edu:8080/thredds/dodsC/models/fvcom/NECOFS/Archive/NECOFS_MASS_BAY/2017/mbn_201707.nc?time[0:1:743]'
        url_201707='http://www.smast.umassd.edu:8080/thredds/dodsC/models/fvcom/NECOFS/Archive/NECOFS_MASS_BAY/2017/mbn_201707.nc?u[0:1:743][0:1:9][0:1:165094],v[0:1:743][0:1:9][0:1:165094]'
        #########################################################   
        d=[]
...
if track_direction=='forward':
    for i in range(len(data['lon'])):
        ...
        ind1=(start_time-datetime(2017,8,1,00,00,00)).days*24+(start_time-datetime(2017,8,1,00,00,00)).seconds/(60*60)
        ind2=(end_time-datetime(2017,8,1,00,00,00)).days*24+(end_time-datetime(2017,8,1,00,00,00)).seconds/(60*60)
        if ind2>743:#the dataset is from (2017,8,1,0,0,0),the total number of data is 744
            ind3=ind2-743#get the index in July        
            ind2=743
        lon_model=np.load('lonc.npy')#massbay model grid point
        lat_model=np.load('latc.npy')#massbay model grid point
        ...
        for c in np.arange(0,days*24+1,1):        
            u_t=ds1['u'][c][0][index3]       
            v_t=ds1['v'][c][0][index3]
            print 'u_t:',u_t,'v_t:',v_t
