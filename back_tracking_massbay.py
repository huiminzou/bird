...
for i in range(len(data['lon'])):#convert the format to degree
    lat1,lon1=dm2dd(data['lat'][i],data['lon'][i])
    lat.append(lat1)
    lon.append(lon1)
for i in range(len(lon)):#plot startpoints
    ax.scatter(lon[i],lat[i],marker='o',color='green',s=8)
...
for a in np.arange(len(lon)):
    ...
    for b in np.arange(len(lon_model)):
        d.append((lon[a]-lon_model[b])*(lon[a]-lon_model[b])+(lat[a]-lat_model[b])*(lat[a]-lat_model[b]))
    index3=np.argmin(d)
    print 'index3',index3
    temlon=lon[a]
    temlat=lat[a]
